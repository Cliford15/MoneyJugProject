...
export default function CreateForm({ selectedJug, onSave, onClose }: CreateFormProps) {
    // initialize from selectedJug so inputs are controlled
    ...
    // update local state when selectedJug changes (important when reusing modal)
    useEffect(() => {
        ...
        try {
            if (selectedJug.id) {
                ...
                if (res.ok) {
                    ...
                    await onSave({ ...selectedJug, name: payload.name, goalAmount: payload.goalAmount, set: true });
                }
            } else {
                // not persisted yet, update locally
                await onSave({ ...selectedJug, name: payload.name, goalAmount: payload.goalAmount, set: true });
            }
        } catch (e) {
            console.error(e);
